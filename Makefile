...
CXXFLAGS=-Werror=return-type -fno-threadsafe-statics -Ilib -Iinc -nostdlib -fno-use-cxa-atexit -ffreestanding -Wall -Wextra -fno-exceptions -fno-rtti
CFLAGS=-Werror=implicit-function-declaration -Werror=return-type -Ilib -Iinc -nostdlib -ffreestanding -Wall -Wextra -fno-exceptions
NASMFLAGS=-felf32
ASFLAGS=--32 -nostdlib
...
run:
	@qemu-system-i386 -cdrom $(ISO_FILE) -serial stdio -vga std -no-reboot -no-shutdown -m 1G
run-kvm:
